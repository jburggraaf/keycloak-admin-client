...
/**
 A function to get the sessions of a user.
 @param {string} realmName - The name of the realm(not the realmID) - ex: master
 @param {string} userId - The id of user
 @returns {Promise} A promise that will resolve with an array of active sessions - empty if none active
 @example
 keycloakAdminClient(settings)
 .then((client) => {
      client.users.sessions(realmName)
        .then((sessions) => {
...
function sessions (client) {
  return function sessions (realm, userId) {
    return new Promise((resolve, reject) => {
      const req = {
        auth: {
          bearer: privates.get(client).accessToken
        ...
        json: true,
        url: `${client.baseUrl}/admin/realms/${realm}/users/${userId}/sessions`
      };
